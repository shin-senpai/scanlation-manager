...
void Commands::removeRoleAutocomplete(Bot &bot, const std::string &key, const std::string &input, const dpp::autocomplete_t &event) {
  ...
  try {
    ...
    std::cerr << "removeRoleAutocomplete failed for key=" << key << ": " << e.what() << std::endl;
  }
