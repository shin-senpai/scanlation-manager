...
void Commands::promote(Bot &bot, const dpp::slashcommand_t &event) {
  event.thinking(true);
  ...
  try {
    ...
    if(current == Permission::supermanager) {
      event.edit_original_response(dpp::message("<@" + std::to_string(target_discord_id) + "> is already a Supermanager."));
      return;
    }

    const Permission next = (current == Permission::standard) ? Permission::manager : Permission::supermanager;
    user_repo.setPermissionLevel(session.wtx(), *maybe_target_id, next);
    session.commit();

    event.edit_original_response(dpp::message(
        "<@" + std::to_string(target_discord_id) + "> promoted from " +
        permissionName(current) + " to **" + permissionName(next) + "**."));
  } catch(const std::exception &e) {
    std::cerr << "promote failed for user (" << discord_id << "): " << e.what() << std::endl;
    event.edit_original_response(dpp::message("Failed to promote user. Contact the administrator to resolve this issue."));
  }
}
