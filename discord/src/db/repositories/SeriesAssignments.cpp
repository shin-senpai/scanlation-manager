...
std::vector<std::string> SeriesAssignmentsRepository::listSeriesNamesByUser(pqxx::transaction_base &txn, int user_id) {
  auto results = txn.exec(
      "SELECT DISTINCT s.name FROM series s"
      " WHERE s.id IN ("
      "   SELECT series_id FROM series_assignments WHERE user_id = $1"
      "   UNION"
      "   SELECT c.series_id FROM chapter_assignments ca"
      "   JOIN chapters c ON ca.chapter_id = c.id WHERE ca.user_id = $1"
      " )",
      pqxx::params(txn, user_id));
