...
#include <curl/curl.h>
	...
	public:
		...
		std::string encode(const std::string& unencoded_element) {
			char* encode = curl_easy_escape(curl, unencoded_element.c_str(), static_cast<int>(unencoded_element.length()));
			if (!encode) {
	...
	static size_t WriteCallback(void* contents, size_t size, size_t nmemb, void* userp) {
		(static_cast<std::string*>(static_cast<void*>(userp)))->append(static_cast<char*>(static_cast<void*>(contents)), size * nmemb);
		return size * nmemb;
	}
	...
	static int executeRequest(CurlWrapper& wrapper, const std::string& url, std::string& read_buffer) {
