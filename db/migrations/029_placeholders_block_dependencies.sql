-- A chapter_assignment_placeholder represents an unfilled vacancy for a task:
-- the task IS needed for the chapter, just unassigned. Dependent tasks must
-- therefore treat a placeholder as a blocker, the same way an incomplete
-- assignment would block them.
--
-- This migration replaces outstanding_chapter_assignments so that the available
-- flag becomes FALSE when any prerequisite has a placeholder.
...
WITH prereq_check AS (
  SELECT
    ...
    CASE
      ...
      WHEN EXISTS (
        -- Prerequisite has an outstanding assignment
      ...
      WHEN EXISTS (
        -- Prerequisite has a placeholder (unfilled vacancy — still needed)
        SELECT 1 FROM task_dependencies td
        JOIN chapter_assignment_placeholders cap
          ON cap.chapter_id = ca.chapter_id
         AND cap.task_id    = td.depends_on_task_id
        WHERE td.task_id = ca.task_id
      ) THEN FALSE
      ELSE TRUE
    END AS available,
